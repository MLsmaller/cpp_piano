...
class CityScapesDataset(BaseDataSet):
    ...
    def _load_data(self, index):
        ...
        #---np.asarray和np.array()的不同是np.array()会复制原对象，用asarray更节省内存
...
class CityScapes(BaseDataLoader):
    def __init__(self, data_dir, batch_size, split, crop_size=None, base_size=None, scale=True, num_workers=1, mode='fine', val=False,
        ...
        #---这里相当于是把子类self.dataset,batch_size等属性传给父类BaseDataLoader，初始化父类并继承其属性
        #---刚好父类BaseDataLoader需要的参数值是这几个东西0.0
...
if __name__=='__main__':
    ...
    class Child(Parent):
        def __init__(self):
            print('child')
            #---super显示调用父类并初始化父类，则name='data from Child'，这样同时继承了父类的属性
            super(Child,self).__init__('data from Child')  
            # super(Child,self).__init__()  这种初始化父类就不行，因为至少需要一个name参数
