...
class BaseDataSet(Dataset):
    ...
    def _load_data(self, index):
        #---raise NotImplementedError语句会要求去调用其子类(即CityScapesDataset)的_load_data()函数
        #---如果子类中没有写该函数的实现则会报错
        raise NotImplementedError
    ...
    def _val_augmentation(self, image, label):
        if self.crop_size:
            ...
            #---将最短边resize成crop_size，另外一个边则是crop_size*w/h,保持着原来的长宽比
            if h < w:
    ...
    def _augmentation(self, image, label):
        ...
        while True: # The rotated crop must have some objects
            ...
            if self.crop_size:
                #---如果img小于crop_size，则padding至crop_size
                ...
                # Cropping 
                #---如果img大于crop_size，则随机从img进行crop到crop_size大小
                h, w, _ = image_new.shape
