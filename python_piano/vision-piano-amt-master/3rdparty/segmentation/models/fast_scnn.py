...
class Fast_SCNN(torch.nn.Module):
    def __init__(self, num_classes,input_channel=3,freeze_bn=False):
        super().__init__()
    ...
    def get_backbone_params(self):
        return chain(self.learning_to_downsample.parameters())
    def get_decoder_params(self):
        return chain(self.global_feature_extractor.parameters(),
...
class FeatureFusionModule(torch.nn.Module):
    ...
    def forward(self, high_res_input, low_res_input):
        input_size = (high_res_input.size()[2],high_res_input.size()[3])
        low_res_input = F.interpolate(input=low_res_input, size=input_size, mode='bilinear', align_corners=True)
