...
def match(threshold, truths, priors, variances, cls_labels, lr_labels,loc_t, cls_conf_t, lr_conf_t,idx):
    ...
    cls_conf = cls_labels[best_truth_idx]         # Shape: [num_priors]
    cls_conf[best_truth_overlap < _th2] = 0  # label as background

  
    lr_conf = lr_labels[best_truth_idx]
    lr_conf[best_truth_overlap<_th2] = 0 
    ...
    if N > 0:
        ...
        cls_conf[stage2_idx[:N]] += 1
        lr_conf[stage2_idx[:N]]+=1 

    loc = encode(matches, priors, variances)
    loc_t[idx] = loc    # [num_priors,4] encoded offsets to learn
    cls_conf_t[idx] = cls_conf  # [num_priors] top class label for each prior
    lr_conf_t[idx] = lr_conf
